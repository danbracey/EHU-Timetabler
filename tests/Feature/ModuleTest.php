...
class ModuleTest extends TestCase
{
    ...
    {
        $response = $this->actingAs($this->user)->post(route('module.timeslot.create'));
    ...
    public function test_manual_timeslot_can_be_edited_for_module(): void
    {
        ...
        $response = $this->actingAs($this->user)->patch(route('module.timeslot.update', $module->__get('id')), [
            'room_id' => $room->__get('id'),
        ...
        // Assert against the updated values
        $this->assertSame('room_id', $module->__get('room_id'));
        $this->assertSame($dayOfWeek, $dayOfWeek);
        $this->assertSame('13:00', $module->__get('start_time'));
        $this->assertSame('14:00', $module->__get('end_time'));
    }

    public function test_manual_timeslot_can_be_deleted_for_module(): void
    {
        $module = Module::factory()->createOne();
        $response = $this->actingAs($this->user)->delete(route('module.timeslot.destroy', $module->__get('id')));
